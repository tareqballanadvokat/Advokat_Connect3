  ...
  socket.onmessage = async event => {
    ...
    logger.log('📥 Otrzymano:\n' + data); 
        ...
        logger.log('Wysyłanie :\n' + request); 
			...
			if (registrationObj.IsRegistrationProcessFinished)
			{
				establishingConnectionObject.updateData(registrationObj.tag, 
				registrationObj.callId,
				registrationObj.branch,
				registrationObj.fromDisplayName,//fromUri, sipUri
		...
		if (establishingConnectionObject.ConnectionType=="OFFER" && peer2PeerConnectionObject.isOfferSent==false)
		{
			...
			const reCallId = /^Call-ID:\s*([^\r\n]+)/m;
			const m = data.match(reCallId); 
			await peer2PeerConnectionObject.createAndSendOffer(socket,m[1],establishingConnectionObject.sipUri, establishingConnectionObject.tag, registrationObj.toLineReplaced);
		}
