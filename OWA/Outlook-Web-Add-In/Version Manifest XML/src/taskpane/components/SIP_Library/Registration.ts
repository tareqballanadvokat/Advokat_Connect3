...
export interface RegistrationEvents
  extends Omit<SipPhaseEvents<RegistrationState, "RECEIVE_TIMEOUT">, "onSuccess" | "onFailure"> {
...
export class Registration {
  ...
  private handleReceiveTimeout(): void {
    ...
    if (this.registrationState !== RegistrationState.REGISTER_SENT) {
      logger.debug(
        `RECEIVE_TIMEOUT fired but state is ${this.registrationState} - ignoring`,
        "Registration"
      );
      return;
    }
  ...
  public resetRegistrationState(): void {
    ...
    logger.info(
      `New session - Call-ID: ${this.callId}, Tag: ${this.tag}, Branch: ${this.branch}`,
      "Registration"
    );
  }
  ...
  private validateSession(data: string): boolean {
    ...
    if (messageCallId !== this.callId) {
      logger.warn(
        `Call-ID mismatch - Expected: ${this.callId}, Got: ${messageCallId} - IGNORING`,
        "Registration"
      );
      return false;
    }
    ...
    if (this.expectedToTag !== "") {
      ...
      if (fromTagMatch && fromTagMatch[1] !== this.expectedToTag) {
        logger.warn(
          `From-tag mismatch - Expected: ${this.expectedToTag}, Got: ${fromTagMatch[1]}`,
          "Registration"
        );
        return false;
      }
  ...
  private parseServerTimeouts(data: string): void {
    ...
    try {
      ...
      if (config.PeerRegistrationTimeout && typeof config.PeerRegistrationTimeout === "number") {
        ...
        logger.debug(
          `PeerRegistrationTimeout set to ${this.peerRegistrationTimeout}ms`,
          "Registration"
        );
      } else {
        logger.debug(
          `Server sent null/invalid PeerRegistrationTimeout, using default: ${this.peerRegistrationTimeout}ms`,
          "Registration"
        );
      }
  ...
  private handle202Accepted(data: string): string {
    ...
    if (fromTagMatch) {
      ...
      logger.warn(
        "WARNING: Server did not provide tag in 202 response From header",
        "Registration"
      );
    }
    ...
    logger.info(
      "Registration phase COMPLETED - transitioning to Connection Establishment",
      "Registration"
    );
  ...
  private extractFromParts(data: string): void {
    ...
    if (m) {
      ...
      logger.debug(
        `Extracted FROM - URI: ${this.parsedFromUri}, Tag: ${this.parsedFromTag}`,
        "Registration"
      );
    } else {
