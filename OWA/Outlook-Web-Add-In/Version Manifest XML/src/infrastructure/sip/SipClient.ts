...
export function initializeSipClient(config?: Partial<SipClientConfig>): SipClientInstance {
  ...
  function isConnected(): boolean {
    return (
      clientState === SipClientState.CONNECTING_P2P || clientState === SipClientState.CONNECTED
    );
  }
  ...
  function sendMessage(message: string, context: string): void {
    ...
    if (
      clientState === SipClientState.FAILED_PERMANENTLY ||
  ...
  function closeConnection(
    reason: string,
    targetState: SipClientState = SipClientState.DISCONNECTED,
  ...
  const registrationEvents: RegistrationEvents = {
    ...
    onSuccess: (timeoutConfig) => {
      ...
      if (!peerRegistrationTimeoutStarted) {
        logWithPrefix(
          `⏱️ Starting PEER_REGISTRATION_TIMEOUT (${timeoutConfig.peerRegistration}ms)`
        );
      ...
      peer2PeerConnectionObject.updateConfiguration(timeoutManager, timeoutConfig.receive);
    },
      ...
      if (canRetryConnection()) {
        ...
        if (canRetryRegistration()) {
          ...
          transitionClientState(
            ...
            "Registration restart after connection failure"
          );
        } else {
          logWithPrefix("❌ Max registration retries reached");
          closeConnection(
            "Max registration retries after connection failure",
            SipClientState.FAILED_PERMANENTLY,
            1000
          );
        }
  ...
  const peer2PeerEvents: Peer2PeerEvents = {
    ...
    onFailure: (reason) => {
      ...
      if (canRetryWebRTC()) {
        ...
        const notRegistered =
          clientState === SipClientState.DISCONNECTED || clientState === SipClientState.FAILED;
        ...
        if (notConnectedAndNotConnecting && notRegistered) {
          ...
          if (canRetryRegistration()) {
            ...
            registrationRetryCount++;
            logWithPrefix(
              `🔄 Not connected/registered - restarting registration (${registrationRetryCount}/${maxRetries})`
            );
            resetForNewRegistration();
            ...
            transitionClientState(
              SipClientState.REGISTERING,
              "Registration restart after WebRTC failure"
            );
          } else {
            // Max registration retries reached - fail completely
            logWithPrefix(
              `❌ Max registration retries (${maxRetries}) reached - closing connection`
            );
            closeConnection(
              "Max registration retries after WebRTC failure",
              SipClientState.FAILED_PERMANENTLY,
              1000
            );
          }
          ...
          logWithPrefix(
            `❌ Max WebRTC retries (${maxRetries}) reached while connected - closing connection`
          );
          closeConnection(
            `SDP exchange failed after ${maxRetries} attempts`,
            SipClientState.FAILED_PERMANENTLY,
            1000
          );
          ...
          logWithPrefix(
            "⏳ Still establishing connection - waiting for connection phase to complete"
          );
  ...
  function handlePeerRegistrationTimeout(): void {
    ...
    if (canRetryRegistration()) {
      ...
      logWithPrefix("❌ Max registration retries reached");
      closeConnection(
        "Max registration retries after peer timeout",
        SipClientState.FAILED_PERMANENTLY,
        1000
      );
    }
    ...
    ws.onmessage = async (event) => {
      ...
      if (isByeMessage) {
        ...
        if (REGEX_REASON_REGISTRATION.test(data)) {
          ...
          if (lastSentCSeq > 0 && incomingCSeq > lastSentCSeq) {
            logWithPrefix(
              `✅ REGISTRATION BYE is a response (CSeq ${incomingCSeq} > ${lastSentCSeq}) - ignoring`
            );
            return;
          }

          logWithPrefix(
            `📥 REGISTRATION BYE received (CSeq: ${incomingCSeq}) - canceling PEER_REGISTRATION_TIMEOUT`
          );
          timeoutManager.cancelTimer(TIMER_PEER_REGISTRATION);
      ...
      switch (clientState) {
        ...
        case SipClientState.CONNECTING: {
          ...
          if (!isRegistered()) {
            logWithPrefix(
              "⚠️ Messages in connection establishment phase rejected - not registered (after REGISTRATION BYE or timeout)"
            );
            break;
          }
  ...
  const sipClientInstance: SipClientInstance = {
    ...
    disconnect: (targetState: SipClientState = SipClientState.DISCONNECTED): void => {
      ...
      const reason =
        targetState === SipClientState.FAILED
