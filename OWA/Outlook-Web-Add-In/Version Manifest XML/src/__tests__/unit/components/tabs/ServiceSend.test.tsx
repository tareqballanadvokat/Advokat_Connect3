...
/**
 * Component Tests for tabs/service/ServiceSend.tsx
 *
...
import ServiceSend from "@components/tabs/service/ServiceSend";

describe("ServiceSend", () => {
  const onTransfer = jest.fn();

  beforeEach(() => jest.clearAllMocks());

  it("displays the case id in a read-only input", () => {
    render(<ServiceSend caseId="TEST-1" onTransfer={onTransfer} transferBtnDisable={false} />);
  ...
  it("disables the button when transferBtnDisable is true", () => {
    render(<ServiceSend caseId="TEST-1" onTransfer={onTransfer} transferBtnDisable={true} />);
  ...
  it("shows the sending label and disables the button while transferLoading", () => {
    render(
      <ServiceSend caseId="TEST-1" onTransfer={onTransfer} transferBtnDisable={false} transferLoading />
    );
