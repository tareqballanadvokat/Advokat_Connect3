...
export class TokenService {
  ...
  async encryptAuthResponse(authResponse: IAuthResponse): Promise<IAuthResponse> {
    ...
    this.logger.debug('Auth response encrypted, will be stored in Redux', 'TokenService');
  ...
  async getCurrentToken(): Promise<string | null> {
    ...
    if (!encryptedToken) {
      this.logger.debug('No encrypted token in store', 'TokenService');
      return null;
    }
    ...
    if (decryptedToken) {
      this.logger.debug('Decrypted token (first 30 / last 30 chars): ' + decryptedToken.substring(0, 30) + '...' + decryptedToken.substring(decryptedToken.length - 30), 'TokenService');
    } else {
      this.logger.debug('Failed to decrypt token', 'TokenService');
    }
