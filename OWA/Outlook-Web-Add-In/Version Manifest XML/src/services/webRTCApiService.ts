...
import { IAuthRequest, IAuthResponse, IAdvokatAuthResponse } from "@interfaces/IAuth";
...
export class WebRTCApiService implements DataChannelObserver {
  ...
  /**
   * First-time OTP pairing (Part 1, Step 7 of the auth flow).
   * Sends { type: 'REGISTER_OTP', otp, officeToken } through the WebRTC data channel.
   * The ADVOKAT Server validates the OTP + Office token, stores oid → SB permanently,
   * registers oid ↔ advokatServerId on the Pairing API, then returns an advokatToken.
   *
   * @param otp          Short-lived code generated by the ADVOKAT Desktop Client
   * @param officeToken  Microsoft-signed JWT from OfficeRuntime.auth.getAccessToken()
   * @returns advokatToken for use during this session (held in Redux memory only)
   */
  async sendRegisterOtpMessage(otp: string, officeToken: string): Promise<IAdvokatAuthResponse> {
    this.logger.info('Sending REGISTER_OTP message through WebRTC data channel...', 'WebRTCApiService');
    ...
    const response = await this.sendRequest(
      ...
      'addin/register-otp',
      { 'Content-Type': 'application/json', Accept: 'application/json' },
      JSON.stringify({ type: 'REGISTER_OTP', otp, officeToken }),
    );
    ...
    if (response.body) {
      const parsed = typeof response.body === 'string'
        ? JSON.parse(response.body) as IAdvokatAuthResponse
        : response.body as IAdvokatAuthResponse;
      this.logger.info('REGISTER_OTP successful — advokatToken received', 'WebRTCApiService');
      return parsed;
    }

    throw new Error('REGISTER_OTP response contained no body');
  }
